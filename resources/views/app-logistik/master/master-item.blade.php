                ...
                <div class="row gx-0 flex-between-center" style="color: white !important;">
                    ...
                    <div class="col-xl-auto px-3 py-2">
                        ...
                        <h4 class="text-white fw-bold mb-0" style="color: white !important;">Master <span
                                class="text-white fw-medium" style="color: white !important;">Item</span>
                        </h4>
                    </div>
                </div>
            </div>
    ...
    <div class="card mb-3">
        <div class="card-header bg-primary">
            <div class="row align-items-center">
                <div class="col">
                    <h3 class="m-0"><span class="badge bg-primary m-0 p-0">Data Master Items</span></h3>
                </div>
                <div class="col-auto">
                    <div class="btn-group" role="group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop2">
                            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal-product-xl"
                                id="button-add-item" data-code="123"><span class="far fa-edit"></span>
                                Add New Item</button>
                            <div class="dropdown-divider"></div>
                            <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal-product-xl"
                                id="button-upload-data-item" data-code="123"><span class="fas fa-upload"></span>
                                Import Excel</button>
                        </div>
                    </div>
                </div>
...
@section('base.js')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            var table = $('#example').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('master_logistik_data_item') }}",
                columns: [{
                    data: 'id',
                    "width": "4%"
                },
    ...
    <script>
        $(document).on("click", "#button-add-item", function (e) {
            ...
            $.ajax({
                url: "{{ route('master_logistik_add_item') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
        ...
        $(document).on("click", "#button-clear-data", function (e) {
            ...
            $.ajax({
                url: "{{ route('master_logistik_add_item_clear_file') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
        ...
        $(document).on("click", "#button-simpan-data", function (e) {
            e.preventDefault();
            var data = $("#form-add-data-item").serialize();
            ...
            $.ajax({
                url: "{{ route('master_logistik_save_item') }}",
                type: "POST",
                cache: false,
                data: data,
                dataType: 'html',
        ...
        $(document).on("click", "#button-upload-data-item", function (e) {
            e.preventDefault();
            $('#menu-product-xl').html(
                '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
            );
            $.ajax({
                url: "{{ route('master_logistik_save_item_upload_file') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
