        ...
        <div class="card bg-200 shadow border border-success">
            <div class="row gx-0 flex-between-center">
                ...
                <div class="col-xl-auto px-3 py-2">
                    ...
                    <h4 class="text-success fw-bold mb-0">Master <span class="text-success fw-medium">Simpanan Pokok</span>
                    </h4>
                </div>
            </div>
        </div>
    ...
    <div class="card-body border-top p-3" id="hasil-pencarian-list">
        <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody class="fs--2">
                ...
                <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $datas->kop_simpanan_pokok_code }}</td>
                    <td>{{ $datas->kop_simpanan_pokok_name }}</td>
                    <td>@currency($datas->kop_simpanan_pokok_nominal )</td>
                    <td></td>
                </tr>
                @endforeach
            </tbody>
...
<script>
    $(document).on("click", "#button-add-data-simpanan", function(e) {
        ...
        $.ajax({
            url: "{{ route('master_koperasi_simpanan_pokok_add') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#button-simpan-data-simpanan", function(e) {
        e.preventDefault();
        var data = $("#form-simpanan-pokok-baru").serialize();
        $('#menu-add-data-simpanan').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        );
        $.ajax({
            url: "{{ route('master_koperasi_simpanan_pokok_save') }}",
            type: "POST",
            cache: false,
            data: data,
            dataType: 'html',
