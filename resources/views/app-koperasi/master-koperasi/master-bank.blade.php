        ...
        <div class="card bg-200 shadow border border-success">
            <div class="row gx-0 flex-between-center">
                ...
                <div class="col-xl-auto px-3 py-2">
                    ...
                    <h4 class="text-success fw-bold mb-0">Master <span class="text-success fw-medium">Data Bank</span>
                    </h4>
                </div>
            </div>
        </div>
...
<div class="card mb-3">
    <div class="card-header bg-primary">
        <div class="d-flex justify-content-between">
            <div>
                <a class="btn btn-falcon-default btn-sm" href="#" data-bs-toggle="modal" data-bs-target="#modal-koperasi" id="button-add-data-bank">
        ...
        <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody class="fs--2">
                ...
                @foreach ($data as $datas)
                    <tr>
                        <td>{{ $no++ }}</td>
                        <td>{{ $datas->kop_master_bank_id }}</td>
                        <td>{{ $datas->kop_master_bank_name }}</td>
                        <td>{{ $datas->kop_master_bank_number }}</td>
                        <td></td>
                    </tr>
                @endforeach
            </tbody>
...
<script>
    $(document).on("click", "#button-add-data-bank", function(e) {
        e.preventDefault();
        $('#menu-koperasi').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        );
        $.ajax({
            url: "{{ route('master_koperasi_data_bank_add') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#button-simpan-data-bank", function(e) {
        e.preventDefault();
        var data = $("#form-bank-baru").serialize();
        $('#menu-add-data-bank').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        );
        $.ajax({
            url: "{{ route('master_koperasi_data_bank_save') }}",
            type: "POST",
            cache: false,
            data: data,
            dataType: 'html',
            ...
            if (data == 0) {
                ...
                $('#menu-add-data-bank').html('<button class="btn btn-success float-end" id="button-simpan-data-cabang" data-code="">Simpan Data</button>');
            } else {
                $('#menu-add-data-bank').html(data);
                location.reload();
            }
        }).fail(function() {
