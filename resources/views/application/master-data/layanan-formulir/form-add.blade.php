<div class="modal-body p-0">
    <div class="bg-300 rounded-top-lg py-3 ps-4 pe-6">
        <h4 class="mb-1" id="staticBackdropLabel">Add Formulir Layanan</h4>
        <p class="fs--2 mb-0">Support by <a class="link-600 fw-semi-bold" href="#!">Transforma</a></p>
    </div>
    <form class="row g-3 p-4" action="{{ route('master_layanan_formulir_save') }}" method="post"
        ...
        <div class="col-6">
            ...
            <select name="code" class="form-control choices-single-jenis" id="">
                ...
                @foreach ($layanan as $lay)
                    <option value="{{ $lay->t_layanan_data_code }}">{{ $lay->t_layanan_data_name }}</option>
                @endforeach
            </select>
        </div>
        <div class="col-6">
            <label class="form-label" for="inputAddress">Nama Formulir</label>
            <input class="form-control form-control-lg" id="inputAddress" type="text" name="name"
                placeholder="Fromulir 1" required />
        </div>
        <div class="col-12">
