          ...
          <tbody>
              ...
              <tr class="border-bottom">
                  <th class="ps-0 pt-0 text-primary">{{$lists->t_pemeriksaan_list_name}}
                      <div class="text-400 fw-normal fs--2">Normal Price : @currency($lists->order_rad_log_price) Disc. {{$lists->order_rad_log_discount}} %</div>
                  </th>
                  <th>
                      <button class="btn btn-danger btn-sm" id="button-remove-data-pemeriksaan-pasien" data-code="{{ $lists->order_rad_log_code }}" data-reg="{{ $lists->d_reg_order_rad_code }}"><span class="fas fa-trash-alt"></span></button>
                  </th>
                  <th class="pe-0 text-end pt-0">@currency($lists->order_rad_log_price - ($lists->order_rad_log_discount*$lists->order_rad_log_price/100))</th>
              </tr>
              @php
              $total = $total + $lists->order_rad_log_price - ($lists->order_rad_log_discount*$lists->order_rad_log_price/100);
              @endphp
              @endforeach
          </tbody>
          <!-- <tr class="border-bottom">
  ...
  <div class="row card-body">
      ...
      <div class="col-md-5 col-xl-12 col-xxl-5 ps-lg-4 ps-xl-2 ps-xxl-5 text-center text-md-start text-xl-center text-xxl-start">
          ...
          <button class="btn btn-success mt-3 px-5" type="submit" id="button-fix-registrasi-rad">Confirm &amp; Create Bill</button>
