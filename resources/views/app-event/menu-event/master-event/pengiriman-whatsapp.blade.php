...
<div class="card border-0 shadow-sm rounded-4 mb-4">
    <div class="card-body p-4">
        <h5 class="fw-bold mb-3 text-dark">
            <i class="fab fa-whatsapp text-success me-2"></i> Pengiriman WhatsApp Tiket Peserta
        </h5>
...
<div class="card border-0 shadow-sm rounded-4" id="wrapperTablePeserta" style="display: none;">
    <div class="card-header bg-white py-3 border-0 d-flex flex-wrap align-items-center justify-content-between gap-2">
        ...
        <!-- Filter Status WA & Tombol Kirim Massal -->
        <div class="d-flex align-items-center gap-2 flex-wrap">
            <div class="btn-group btn-group-sm" role="group" aria-label="Filter Status WhatsApp">
                <input type="radio" class="btn-check" name="filterStatusWa" id="filterAll" value="all" checked autocomplete="off">
                <label class="btn btn-outline-secondary px-3" for="filterAll">Semua Peserta</label>

                <input type="radio" class="btn-check" name="filterStatusWa" id="filterUnsent" value="unsent" autocomplete="off">
                <label class="btn btn-outline-warning text-dark fw-semibold px-3" for="filterUnsent">Belum Terkirim</label>
            </div>

            <button class="btn btn-sm btn-success rounded-3 px-3 py-2 fw-semibold" id="btnKirimSemua" onclick="sendWaBulk()">
                <i class="fab fa-whatsapp me-1"></i> Kirim Semua WA
            </button>
        </div>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#selectEvent').on('change', function() {
            ...
            if (eventId) {
                $.ajax({
                    url: `/event-wa/get-sub-events/${eventId}`,
                    type: 'GET',
        ...
        $('#selectSubEvent').on('change', function() {
            ...
            if (subCode) {
                $.ajax({
                    url: `/event-wa/get-classes/${subCode}`,
                    type: 'GET',
                    success: function(data) {
                        if (data && data.length > 0) {
        ...
        $('input[name="filterStatusWa"]').on('change', function() {
            renderTableData();
        });
    });
    ...
    function loadParticipants(subCode, classId) {
        ...
        $('#tbodyPeserta').html('<tr><td colspan="8" class="text-center py-4"><div class="spinner-border text-success"></div><div class="mt-2 text-muted">Memuat data peserta...</div></td></tr>');

        $.ajax({
            url: '{{ route("event.wa.get_participants") }}',
            type: 'GET',
            data: {
                sub_code: subCode,
                class_id: classId
    ...
    function renderTableData() {
        ...
        let filterType = $('input[name="filterStatusWa"]:checked').val();
        let displayData = rawParticipantsData;

        if (filterType === 'unsent') {
            displayData = rawParticipantsData.filter(item => !item.wa_sent_at);
        }
        ...
        if (displayData.length > 0) {
            $.each(displayData, function(i, item) {
                ...
                // Badge Status Pengiriman WhatsApp
                let waStatusBadge = item.wa_sent_at ?
                    `<span class="badge bg-success text-white" title="${item.wa_sent_at}"><i class="fas fa-check-circle me-1"></i> Terkirim</span>` :
                ...
                <tr>
                    ...
                    <td class="text-center">
                        <button class="btn btn-sm btn-success rounded-3 me-1" onclick="sendWaSingle('${item.id_registration}', '${item.full_name}')">
                            <i class="fab fa-whatsapp me-1"></i> Kirim WA
                        </button>
                        ${phoneFormatted ? `
                        <a href="https://wa.me/${phoneFormatted}" target="_blank" class="btn btn-sm btn-outline-success rounded-3" title="Chat via WhatsApp Web">
            ...
            let msg = filterType === 'unsent' ? 'Semua peserta sudah menerima pesan WhatsApp.' : 'Tidak ada data peserta.';
            $('#tbodyPeserta').html(`<tr><td colspan="8" class="text-center py-4 text-muted">${msg}</td></tr>`);
        }
    }

    function sendWaSingle(idRegistration, name) {
        Swal.fire({
            title: 'Kirim WA Tiket?',
            text: 'Sistem akan mengirimkan pesan WhatsApp ke ' + name,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: `/event-wa/send-single/${idRegistration}`,
                    type: 'POST',
                    ...
                    error: function(xhr) {
                        Swal.fire('Gagal!', xhr.responseJSON?.message || 'Gagal mengirim WhatsApp.', 'error');
                    }
                });
            }
        });
    ...
    function sendWaBulk() {
        ...
        if (filterType === 'unsent') {
            targetList = rawParticipantsData.filter(item => !item.wa_sent_at);
        } else {
            targetList = rawParticipantsData.filter(item => !item.wa_sent_at);
        }

        if (targetList.length === 0) {
            Swal.fire('Info', 'Semua peserta pada daftar ini sudah menerima pesan WhatsApp.', 'info');
            return;
        }
        ...
        Swal.fire({
            ...
            text: `Sistem akan mengirim WhatsApp ke ${targetIds.length} peserta yang belum terkirim. Lanjutkan?`,
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        Swal.fire('Gagal!', xhr.responseJSON?.message || 'Gagal mengirim WhatsApp massal.', 'error');
                    }
                });
            }
        });
