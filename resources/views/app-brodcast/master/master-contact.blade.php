            ...
            <div class="row gx-0 flex-between-center">
                ...
                <div class="col-xl-auto px-3 py-2">
                    ...
                    <h4 class="text-white fw-bold mb-0" style="color: white !important;">Master <span
                            class="text-white fw-medium" style="color: white !important;">Contact</span>
                    </h4>
                </div>
            </div>
        </div>
...
<div class="card mb-3">
    <div class="card-header bg-primary">
        <div class="row align-items-center">
            <div class="col">
                <h3 class="m-0"><span class="badge bg-primary m-0 p-0">Master Contact</span></h3>
            </div>
            <div class="col-auto">
    ...
    <div class="card-body border-top p-3">
        <table id="example" class="table table-striped" style="width:100%">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $no++ }}</td>
                    <td>{{ $datas->b_master_contact_name }}</td>
                    <td>{{ $datas->b_master_contact_email }}</td>
                    <td>{{ $datas->b_master_contact_whatsapp }}</td>
                    <td>{{ $datas->b_master_contact_status }}</td>
                    <td class="text-center">
                        <div class="btn-group" role="group">
                            <button class="btn btn-sm btn-falcon-primary dropdown-toggle" id="btnGroupVerticalDrop2"
                                type="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false"><span
                                    class="fas fa-align-left me-1" data-fa-transform="shrink-3"></span>Action</button>
                            <div class="dropdown-menu" aria-labelledby="btnGroupVerticalDrop2">
                                <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#modal-brodcast"
                                    id="button-update-contact" data-code="{{$datas->b_master_contact_code}}"><span class="far fa-edit"></span>
...
<script>
    $(document).on("click", "#button-add-contact", function(e) {
        e.preventDefault();
        $('#menu-brodcast').html(
    ...
    $(document).on("click", "#button-simpan-data-contact", function(e) {
        e.preventDefault();
        var data = $("#form-input-contact").serialize();
        $('#menu-add-data-contact').html(
            '<div class="spinner-border my-3" style="display: block; margin-left: auto; margin-right: auto;" role="status"><span class="visually-hidden">Loading...</span></div>'
        );
        $.ajax({
            url: "{{ route('master_brodcast_contact_save') }}",
            type: "POST",
            cache: false,
            data: data,
            dataType: 'html',
            ...
            if (data == 0) {
                ...
                $('#menu-add-data-contact').html('<button class="btn btn-success float-end" id="button-simpan-data-contact" data-code="">Simpan Data</button>');
            } else {
                $('#menu-add-data-contact').html(data);
                location.reload();
            }
        }).fail(function() {
            $('#menu-add-data-contact').html('eror');
        });
    });
</script>
@endsection
